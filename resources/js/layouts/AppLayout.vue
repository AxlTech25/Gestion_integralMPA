    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <header class="flex justify-between items-center w-full px-container-padding h-16 bg-surface border-b border-outline-variant z-40 shrink-0">
        <div class="flex items-center gap-4 min-w-0">
          ...
          <nav v-if="isCalidad" class="hidden lg:flex gap-6 ml-4">
            <router-link
              v-for="sub in calidadNav"
              :key="sub.key"
              :to="{ name: sub.route }"
              custom
              v-slot="{ href, navigate, isActive }"
...
const isCalidad = computed(() => route.path.includes('/admin/calidad'));

const calidadNav = computed(() => auth.calidadMenu ?? []);
...
function isSectionActive(item) {
    ...
    if (item.key === 'calidad') return isCalidad.value;
    return false;
}
